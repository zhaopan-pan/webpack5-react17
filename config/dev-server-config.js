...
function devServerConfig(app, defaultUrl) {
    webpackConfig.entry.bundle.unshift(
        'webpack-hot-middleware/client?reload=true&path=/__webpack_hmr&noInfo=true' // whm的client用路由传参来控制
    )
    let compiler
    try {
    ...
    //技术选型中使用了 webpack-dev-middleware 而没有使用 webpack-dev-server，
    //so使用 webpack-hot-middleware 依赖包,以在自定义服务器或应用程序上启用 HMR。
    app.use(
    ...
    compiler.hooks.done.tap(
        ...
        (stats /* stats is passed as an argument when done hook is tapped.  */) => {
            ...
            if (!hasErrors && !hasWarnings) {
                console.log(
                    chalk.green('==> 🌎 Open up ' + chalk.yellow(defaultUrl) + ' in your browser.')
                )
            }
        }
    )
